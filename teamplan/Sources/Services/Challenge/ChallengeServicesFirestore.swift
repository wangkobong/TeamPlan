...
extension ChallengeServicesFirestore {
    func fetchInfoDocsSnapshotList() async throws -> [QueryDocumentSnapshot] {
        let query = getFirestoreInstance()
            .collection(CollectionType.challengeInfo.rawValue)
        return try await query.getDocuments().documents
    }
    
    func fetchStatusDocsSnapshotList(with userId: String) async throws -> [QueryDocumentSnapshot] {
        let query = getFirestoreInstance()
            .collection(CollectionType.challengeStatus.rawValue)
            .document(userId)
            .collection(CollectionType.challengeStatus.rawValue)
    ...
    func fetchStatusDocsReference(with userId: String, and challengeId: Int) async throws -> DocumentReference {
        let query = getFirestoreInstance()
            .collection(CollectionType.challengeStatus.rawValue)
            .document(userId)
            .collection(CollectionType.challengeStatus.rawValue)
